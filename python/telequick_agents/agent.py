...
class Call:
    ...
    def send_tool_call(
        ...
        """Report a tool you invoked → analytics timeline +
        ``voice.agent.tool_called`` webhooks. REDACT sensitive values yourself:
        args/result are forwarded verbatim."""
        self._session.send_tool_call(tool, args, result)

    def send_usage(
        self, stage: str, fields: dict | None = None, provider: str = "", model: str = ""
    ) -> None:
        """Report per-stage usage ('stt'/'llm'/'tts') → token analytics. You run
        the providers, so only what you forward is counted."""
        self._session.send_usage(stage, fields, provider, model)
