...
// SendToolCall reports a tool you invoked → analytics timeline +
// voice.agent.tool_called webhooks. REDACT sensitive values yourself:
// args/result are forwarded verbatim.
func (c *Call) SendToolCall(tool string, args, result map[string]any) {
	c.sess.SendToolCall(tool, args, result)
}

// SendUsage reports per-stage usage ("stt"/"llm"/"tts") → token analytics. You
// run the providers, so only what you forward is counted.
func (c *Call) SendUsage(stage string, fields map[string]any, provider, model string) {
	c.sess.SendUsage(stage, fields, provider, model)
}
