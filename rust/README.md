...
- **Media plane** — one outbound QUIC/WebTransport session (no inbound ports),
  ...
  `recv_audio`, `send_audio`, `clear` (barge-in), `flush`, `send_transcript`,
...
# Cargo.toml
[dependencies]
telequick-agents = { version = "0.2.0", registry = "telequick" }
```
...
```bash
TELEQUICK_HOST=engine.telequick.dev TELEQUICK_MEDIA_KEY=mk_… \
